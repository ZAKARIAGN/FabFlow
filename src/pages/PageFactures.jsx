...
const PagePickingInvoice = ({ isDark }) => {
  ...
  useEffect(() => {
    const delaySearch = setTimeout(async () => {
      if (!searchTerm.trim()) {
        // Si search vide, charger tous les deliveries
        try {
          const data = await GetValideDeliveries(setErrMsg);
          setDeliveries(data || []);
        } catch (err) {
  ...
  if (loading) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${isDark ? "bg-gray-900" : ""}`}
      >
        <div className="text-center">
          <div
            className={`inline-block animate-spin rounded-full h-12 w-12 border-b-2 ${isDark ? "border-white" : "border-[#094067]"} mb-4`}
          ></div>
          <div
            className={`${isDark ? "text-white" : "text-[#094067]"} font-semibold`}
          >
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col gap-3 lg:flex-row justify-between items-center">
          <div className="flex items-center gap-3">
            <FileText
              className={isDark ? "text-white" : "text-[#094067]"}
              size={32}
            />
            <h1
              className={`lg:text-3xl md:text-3xl text-[20px] font-bold ${isDark ? "text-white" : "text-[#094067]"}`}
            >
              Générer une Facture
            </h1>
          </div>
        ...
        {/* ✅ Search Section - Sans Button */}
        <div
          className={`rounded-2xl shadow-md p-6 ${isDark ? "bg-gray-800 text-gray-300" : "bg-white text-gray-700"}`}
        >
        ...
        >
          ...
          {deliveries.length === 0 ? (
            <div className="text-center py-8">
              <AlertCircle
                className={`mx-auto mb-2 ${isDark ? "text-gray-400" : "text-gray-500"}`}
                size={48}
              />
              <p className={`${isDark ? "text-gray-400" : "text-gray-500"}`}>
                Aucun bon de livraison livré trouvé
              </p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        <div
                          className={`font-bold ${isDark ? "text-white" : "text-[#094067]"}`}
                        >
        ...
        {selectedDelivery && (
          ...
          >
            <div className="mb-6">
              ...
              <div
                className={`grid grid-cols-1 lg:grid-cols-2 gap-4 text-sm p-4 rounded-xl mb-4 ${isDark ? "bg-gray-700" : "bg-gray-50"}`}
              >
                <div>
            ...
            <div className="overflow-x-auto">
              <table
                className={`w-full text-sm border rounded-xl ${isDark ? "border-gray-700" : "border-gray-200"}`}
              >
                <thead
                  className={`text-white ${isDark ? "bg-gray-700" : "bg-[#094067]"}`}
                >
                  <tr>
                  ...
                  {selectedDelivery.items?.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-center">
                        <span
                          className={`px-3 py-1 rounded-lg font-semibold ${isDark ? "bg-gray-600 text-gray-200" : "bg-gray-100 text-gray-800"}`}
                        >
                          {item.qtte}
                        </span>
                      </td>
                      ...
                      <td className="p-3 text-center">
                        <span
                          className={`px-2 py-1 rounded ${isDark ? "bg-gray-600 text-gray-200" : "bg-gray-100 text-gray-700"}`}
                        >
                          {item.tax_rate}%
                        </span>
                      </td>
                      <td
                        className={`p-3 text-right font-semibold ${isDark ? "text-blue-400" : "text-[#094067]"}`}
                      >
              ...
              {(() => {
                ...
                return (
                  <div
                    className={`mt-6 rounded-xl p-4 ${isDark ? "bg-gray-700 text-gray-300" : "bg-gray-50 text-gray-700"}`}
                  >
                    <div className="flex flex-col gap-2 max-w-md ml-auto">
                      ...
                      <div
                        className={`flex justify-between text-lg font-bold pt-2 border-t-2 ${isDark ? "border-blue-500 text-blue-400" : "border-[#094067] text-[#094067]"}`}
                      >
                        <span>Total TTC:</span>
